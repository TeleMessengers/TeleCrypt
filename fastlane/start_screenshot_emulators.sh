...
mkdir "emulator_logs"
emulator -avd Nexus_5 -port 5554 -no-window -no-audio -no-boot-anim -no-accel >> emulator_logs/nexus_5.log 2>&1 &
...
wait_for_emulator() {
    ...
    while true; do
        ...
        out=$(adb -s "emulator-${adb_port}" shell getprop sys.boot_completed | tr -d '\r')
        if [ "$out" = "1" ]; then
            break
        fi
        let "r = sec % 5"
        if [[ $r -eq 0 ]]; then
            echo "Waiting for emulator on port ${adb_port} to start (boot_completed=$out))"
        fi
        sleep 1
        let "sec++"
    done
